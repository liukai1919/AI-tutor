...
function createTools(deps) {
  ...
  T("questions.answerQuiz", "Submit the student's pick (0-3) for the current quiz question, identified by its qid; returns correct/answerIndex/explain and the next question. Resending the last answered qid returns that same reply (replay:true) instead of answering the next question; any other qid is rejected (staleQuestion).",
    obj({ session: { type: "string", minLength: 8, maxLength: 64 }, qid: { type: "string", minLength: 1, maxLength: 64 }, picked: { type: "integer", minimum: 0, maximum: 3 } }, ["session", "qid", "picked"]), BOTH, "write", 3000,
