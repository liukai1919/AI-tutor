 ...
 * trace 是旁路：onTrace（或默认 log）同步抛错 / 返回 reject 的 Promise 都只交给 onTraceError（默认 console.warn），
 * 不改变 invoke 的结果，也不会再补发一次 trace；onTraceError 自己出错同样吞掉。
 */
"use strict";
...
function createRegistry(opts) {
  ...
  const onTraceError = opts.onTraceError || ((e, t) => console.warn(`[tool] trace callback failed for ${t.tool} trace=${t.traceId}: ${(e && e.message) || e}`));
  const tools = new Map();

  function reportTraceError(e, t) {
    try {
      const p = onTraceError(e, t);
      if (p && typeof p.then === "function") p.then(null, () => {});
    } catch (_) { /* 上报通道也坏了：trace 是旁路，吞掉 */ }
  }
  /* 发 trace 且只发一次；回调的任何失败都不影响业务结果 */
  function emitTrace(t) {
    try {
      const p = onTrace(t);
      if (p && typeof p.then === "function") p.then(null, e => reportTraceError(e, t));
  ...
  async function invoke(name, ctx, input) {
    ...
    const fail = (code, message, extra) => {
      ...
      emitTrace({ traceId, tool: name, role: ctx && ctx.role, kidId: ctx && ctx.kidId, ok: false, code, status: error.status, ms: meta.ms, at: t0 });
    ...
    /* 成功的 trace 放在 try 外：它出错不能被当成 run 失败再发一次 trace */
    meta.ms = now() - t0;
    emitTrace({ traceId, tool: name, role: ctx.role, kidId: ctx.kidId, ok: true, ms: meta.ms, at: t0 });
