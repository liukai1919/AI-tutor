 ...
 * qbank 每次现取 deps.qbank，不在创建时攥住：家长清库后题库是新内容，攥着旧对象会让新题一开局就 staleSession（#24 复审）。
 */
"use strict";
...
module.exports = function createQuizActions(deps) {
  const { Q, L, normLang, findCurriculumItem, pickProvider, qbankPlayable, ensureQuizBank, ledgerAdd, shuffleArr,
    quizOpen, quizOpenCreate, quizOpenGet, quizOpenTake, quizNextPublic, qbankKey, qbankSave,
    kidTxn, progressRecord, missRecord, progressStatus, progressLevel, remediationFor } = deps;
  const bankOf = (cid, lang) => deps.qbank[qbankKey(cid, lang)];
  ...
  return {
     ...
     * 必须带 qid（答的是哪道题）：以前只有 session+picked，回包丢了前端重交会被当成下一题的答案（#23 复审）。
     *   qid = 当前题        → 判分、推进
     *   qid = 刚答过的那题  → 重放：原样回那次的结果（replay:true，picked 是第一次记下的选项），不推进、不重判
     *   其它 qid            → 409 staleQuestion，什么都不动
     * 不管是作答还是重放，这道题都得还在当前题库里：家长清库后旧票一律 400 staleSession，
     * 不能拿清库前缓存的回包（旧题的答案、讲解、下一题）冒充当前题库（#24 二次复审） */
    answer(ctx, input) {
      const open = quizOpenGet(input && input.session, ctx.userId, null);
      if (!open) throw new ActionError(400, STALE);
      const qid = typeof input.qid === "string" ? input.qid : "";
      if (!qid) throw new ActionError(400, { error: "作答要带题号 qid / qid of the question being answered is required", needQid: true });
      const st = open.state;
      const fresh = st.cur === qid && !st.answered;
      const replay = !fresh && !!open.lastReply && open.lastReply.qid === qid;
      if (!fresh && !replay) {
        if (!st.cur || st.answered) throw new ActionError(400, { error: "这场已经答完了，去结算吧 / This quiz is over, settle it", finished: true });
        throw new ActionError(409, { error: "这道题不是当前这一题 / That is not the current question", staleQuestion: true, n: st.n });
      }
      const bank = bankOf(open.cid, open.lang);
      const q = bank ? bank.questions.find(x => x.qid === qid) : null;
      if (!q) throw new ActionError(400, { error: "这场的题不在题库里了 / The quiz bank changed underneath this session", staleSession: true });
      if (replay) return Object.assign({}, open.lastReply.body, { replay: true });
      ...
      const body = {
        correct: r.correct, picked: input.picked, answerIndex: q.answerIndex, explain: q.explain || "",
        level: st.level, n: st.n, topRight: st.topRight, finished: r.finished || !next, next
      };
      open.lastReply = { qid, body };
      return body;
    },
    ...
    finish(ctx, input) {
      ...
      if (!open) throw new ActionError(400, STALE);
      const bank = bankOf(cid, open.lang);
      const now = Date.now();
