...
- **规则全在服务端（2026-09-25 起，#23）**：升降级、借题、通关判定由 `lib/domain/quiz.js` 执行，状态装在场次票里。
  `/api/quiz/session` 只发第一题（没有 `answerIndex` / `explain` / `tags`）；`/api/quiz/answer { session, qid, picked }`
  判分并返回答案、讲解和下一题；`/api/quiz/finish { session }` 按票里的作答结算，`body.results` 不再看。
  客户端不再持有题包，也不再自己判通关；结算失败一律显示「没存上」而不是通关。
- **作答绑题号、重放幂等（2026-09-25 起，#23 复审）**：`qid` 必填，缺了 400 `needQid`（不会退回「答当前那题」）。
  - `qid` = 当前题 → 判分、推进，响应多一个 `picked`（记下的选项）。
  - `qid` = 刚答过的那一题 → 重放（回包丢了重交）：原样回那次的结果并带 `replay: true`，`picked` 是第一次记下的选项，**不推进、不重判、不重复记分**；最后一题答完后重放也照样回那次 `finished` 的结果。
  - 其它 `qid`（更早的题、别的题）→ 409 `staleQuestion`，什么都不动。
  - 一场里同一个 qid 只出现一次（`pickSession` 去重，没有 qid 的题不进场），重放和「下一题」不会混。
- **中途退出 / 切语言（前端）**：交出去的答案可能已经记在票上（回包还没到或丢了），所以按「交过没有」而不是「收到几个回包」判断要不要结算；
  有答案在路上就等它落地再凭原场次的票结算，一场只结一次，迟到的回包不改新场次。
  - 开局请求按代次认，语言在发请求时定死：快速来回切语言（en → zh → en）时只有最后一次开局作数，场次语言 = 用户最后选的语言；
    过时的开局回包（不管先到后到）丢掉；等开局时点了退出，迟到的开局不会把闯关重新打开。
  - 切语言重开失败（比如新语言没题库也没引擎）：旧场次照样结算，然后退出闯关回清单并提示错误，不停在点不动的旧题上。
- 清库（`DELETE /api/qbank`）原地清空同一个题库容器：清库前开的票答题 **和重放** 都 400 `staleSession`
  （重放也要先确认那道题还在当前题库里，不拿清库前缓存的回包冒充），结算什么都不记；清库后新出的题正常开局、作答、重放、结算。
