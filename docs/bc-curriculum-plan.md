...
- `seen`：讲过 ≥1 次
- `solid`：练习答对 ≥2 次**且分布在不同日期**，或家长手动标记；P5 起新增第三条路——闯关练习通关（SAT 式自适应做题，规则见 `docs/qbank-standard.md`）
...
## P6 单元测试（已实现，2026-08-14）

- 「单元」= `strandGroups` 的一组：BC 大纲的一条主线，或教材（`type:"book"`）的一章。入口是清单里
  每个单元标题右边的 📝，出的卷子按孩子存 `data/kids/<kidId>/unit-tests.json`，可反复做。
- 出卷规则（提示词 `unitTestPrompt` + 校验 `validateUnitTest`）：
  - 默认 8 题（服务端夹 6~12），难度配比 L1/L2/L3 ≈ 3/3/2（`unitTestMix`），交付前按难度升序排，压轴题在最后；
  - 题目要铺满本单元的知识点（知识点数 ≥ 题数时每点最多 1 题，否则每点至少 1 题）；
  - 每题必须挂**本单元**的 `curriculumId`——挂到别的单元一律置空，置空的题超过一半就整卷重试
    （否则按单元统计就成了摆设）。
- 判分在服务端：`POST /api/unit-test/attempt` 只收「第几题选了第几个」，对错按存档答案算，
  逐题记 `practiced-right/wrong` 到对应知识点。前端报的分数不参与判定。
- 与另外两种练习的分工：⚡ 闯关盯**单个**知识点、通关标 solid（`docs/qbank-standard.md`）；
  🎯 FSA 是跨主线的全省测评风格、只有 G4/G7；📝 单元测试是**一个单元收尾**，任何年级、任何教材都能出。
- 成绩不直接标 solid：一个单元 8 题摊到 5 个知识点，证据量不够（同 FSA，只记 right/wrong，
  靠「不同日期答对 ≥2 次」自然升级）。单元成绩进家长报告的 `unitRecent`。
...
5. **进度归属**：~~目前单孩子（`kidName` 一个）。多孩子家庭 progress 要按 kidName 分桶——schema 预留即可，MVP 不做。~~
   **已实现（2026-08-13）**：账号体系上线（家长注册、孩子由家长创建、登录按角色授权），
   progress/history/fsa-sets/reports 按孩子分桶存 `data/kids/<kidId>/`，旧根目录数据在创建第一个孩子时自动迁入；qbank 保持全家共享。见 README「账号」一节。
