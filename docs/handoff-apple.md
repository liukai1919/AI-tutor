...
零依赖 Node 脚本，改 JSON 后跑一遍：id 文法、主题/标准/先修/误区引用存在、先修无环、**不允许先修指向更高年级**、每条 G4–G7 标准至少 1 个 primary 技能、`rep[0]` 与 `diag` 的 L1 入口一致。`--md` 生成完整目录树。你们用 Swift 解析前可以先用它把关；逻辑也很容易移植成单测。
...
另外 `data/lessons/{zh,en}/YY.MATH.FRAC.EQUIV.SAME_WHOLE.json` 是两节示例微课（4–6 步、接先修、专门演一步误区）；其余 488 节正在烤，烤完一并可用。
